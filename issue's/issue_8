import cv2
import numpy as np
import matplotlib.pyplot as plt
from pd_func import gaussianFilter
# carrega a imagem
img = cv2.imread('figs\images.jpg',
                 cv2.IMREAD_GRAYSCALE)

# mostra a imagem de entrada(imagem cinza)
cv2.imshow('Img', img)
cv2.waitKey(0)
cv2.destroyAllWindows()
# pega as dimensões da foto(tem um comando da open cv que faz isso pra ti)
h, w = img.shape[:2]
print('height', h)
print('weight', w)
# cria uma matriz de zeros do tamanho da foto
h = np.zeros(256)
for i in range(img.shape[0]):
    for j in range(img.shape[1]):
        h[img[i, j]] += 1

eixo_x = list(range(256))
plt.figure(2)
plt.bar(eixo_x, h)
plt.show()


# carrega a imagem
img1 = cv2.imread('figs\images.jpg',
                 cv2.IMREAD_GRAYSCALE)

image_content = 'menina'


kernel_size_list = [3]  # k

number_of_iterations_list = [3]  # n

for k in kernel_size_list:
    for n in number_of_iterations_list:
        filtered_image, filtering_parameters = gaussianFilter(
            image_content, img, k, n, 1)

        print('filtering parameters:', filtering_parameters)

        file_name = './smoothing_filters/results_gaussian_filtering/{}_k{}n{}pdd{}_gaussian_filter.jpg'

        cv2.imwrite(file_name.format(
            filtering_parameters[0],  # image content
            filtering_parameters[1],  # kernel dimensions
            filtering_parameters[2],  # number of iterations
            filtering_parameters[3]),  # padding
            filtered_image)

        cv2.imshow('laplacian', filtered_image)

        cv2.waitKey(0)

# mostra a imagem de entrada(imagem cinza)
cv2.imshow('Img', filtered_image)
cv2.waitKey(0)
cv2.destroyAllWindows()
# pega as dimensões da foto(tem um comando da open cv que faz isso pra ti)
h, w = filtered_image.shape[:2]
print('height', h)
print('weight', w)
# cria uma matriz de zeros do tamanho da foto
h = np.zeros(256)
for i in range(filtered_image.shape[0]):
    for j in range(filtered_image.shape[1]):
        h[filtered_image[i, j]] += 1

eixo_x = list(range(256))
plt.figure(2)
plt.bar(eixo_x, h)
plt.show()
